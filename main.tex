...
The protocol calculates the \term{Maximum Soil} ($S_{t_{q}}^{\text{max}}$), such that $S_{t_{q}}^{\text{max}} \in \{j \times 10^{-6} \mid j \in \mathbb{N} \}$, in each interval $q$ of the \term{Morning} of $t$ for a given $\Delta B_{\Xi-1}^{\text{EMA}}$, $\Delta B_{\overline{t-1}}$, $\mathscr{F}$, $W$, $\mathscr{P}$ and $S_{t_{q}}^{\text{min}}$ as:

    $$
        S_{t_{q}}^{\text{max}} = 
            \begin{cases} 
                \text{min}(-\Delta B_{\Xi-1}^{\text{EMA}}, -\Delta B_{\overline{t-1}}) \times \mathscr{F} \times (1 - W)
                    & \text{if} \; 
