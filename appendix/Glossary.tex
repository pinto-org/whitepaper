...
\begin{itemize}[topsep=0pt, itemsep=3pt,leftmargin=16pt]
    ...
    \item[] \term{Blight Factor} - The value adjusted by a \term{Gauge Implementation} that changes the \term{Mown Stalk} penalty for \term{Converting} \term{Deposited} \Pinto\ to \term{Deposited} LP tokens;
    ...
    \item[] \term{Crop Scalar} - The value adjusted by the target maintenance mechanism in order to change the \term{Crop Ratio};
    \item[] \term{Cultivation Factor} - The value adjusted by a \term{Gauge Implementation} that causes the \term{Soil} issued to increase as Pinto is \term{Sown} into all \term{Soil}, and decrease otherwise. 
    ...
    \item[] ETH - Ether;
    \item[] \term{Farm} - Where Pinto target maintenance and use of Pinto take place;
    ...
    \item[] \term{Furrow Scalar} - A coefficient for \term{Soil} issuance when $\Delta B_{\Xi-1}^{\text{EMA}} < 0$ and $\Delta B_{\overline{t-1}} < 0$;
    \item[] \term{Gauge Implementation} - A system for adjusting a variable at the beginning of each \term{Season};
    ...
    \item[] \term{Soil Diffusion Period} - A coefficient for \term{Soil} issuance when $\Delta B_{\Xi-1}^{\text{EMA}} < 0$ and $\Delta B_{\overline{t-1}} < 0$;
    ...
    \item[] \term{Till Scalar} - A coefficient for \term{Soil} issuance when $\Delta B_{\Xi-1}^{\text{EMA}} > 0$ or $\Delta B_{\overline{t-1}} > 0$;
    \item[] \term{Toolshed} - A suite of tools that decrease friction to use Pinto and participate in target maintenance;
...
\begin{itemize}[topsep=0pt, itemsep=3pt,leftmargin=16pt]
    ...
    \item[] $g^{\mathfrak{D}^{*}}$ - The \term{Mown Stalk} penalty for \term{Converting} \term{Deposited} \Pinto\ to \term{Deposited} LP tokens;
    ...
    \item[] $\overline{S}^{\text{min}}$ - The minimum \term{Soil} issued when $\Delta B_{\Xi-1}^{\text{EMA}} < 0$ and $\Delta B_{\overline{t-1}} < 0$;
...
\begin{itemize}[topsep=0pt, itemsep=3pt,leftmargin=16pt]
    ...
    \item[] $\mathfrak{C}_{t}$ - The \term{Cultivation Factor} during $t$;
    \item[] $\Delta \mathfrak{C}_{t}$ - The change in \term{Cultivation Factor} during $t$;
    \item[] $\mathfrak{D}_{t}$ - The \term{Blight Factor} during $t$;
    \item[] $\mathfrak{D}^{\text{max}}$ - The maximum \term{Blight Factor};
...
\begin{itemize}[topsep=0pt, itemsep=3pt,leftmargin=16pt]
    ...
    \item[] $\mathscr{R}$ - The \term{Soil Diffusion Period};
